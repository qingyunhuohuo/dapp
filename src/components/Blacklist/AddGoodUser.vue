<template>
  <div class="page">
    ...
    <el-dialog
      ...
      width="40%">
      <span>成功将 {{ oneAdd.user }} 设置为权利账户</span>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        user: [
          { required: true, message: '请输入你要添加的账户', trigger: 'blur' },
          { min: 42, max: 42, message: '长度应为42个字符', trigger: 'blur' }
        ],
      },
    }
    ...
    AddUser(oneAdd) {
      this.$refs[oneAdd].validate((valid)=>{
        if(valid){
          ...
          tokenContract.methods.good(this.oneAdd.user).call(null,(error,result)=>{
            if(result == 1){
              this.$message('该用户已是权利账户');
            }
            else{
              tokenContract.methods.owner().call(null, (error, result)=>{
                if(result == this.oneAdd.adder){
                  tokenContract.methods.addGoodUser(this.oneAdd.user).send({
