  ...
  <div class="page">
    <div class="all">
      ...
      <el-form :model="oneDelete" status-icon :rules="rules" ref="oneDelete" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item class="bor" label="权利账户" prop="user">
    ...
    <el-dialog
      ...
      width="40%">
      <span>成功将 {{ oneDelete.user }} 移出权利账户名单</span>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        user: [
          { required: true, message: '请输入你要移除的账户', trigger: 'blur' },
          { min: 42, max: 42, message: '长度应为42个字符', trigger: 'blur' }
        ],
      },
    }
    ...
    AddUser(oneDelete) {
      this.$refs[oneDelete].validate((valid)=>{
        if(valid){
          ...
          tokenContract.methods.good(this.oneDelete.user).call(null,(error,result)=>{
            if(result == 0){
              this.$message('该用户不是权利账户');
            }
            else{
              tokenContract.methods.owner().call(null, (error, result)=>{
                if(result == this.oneDelete.adder){
                  tokenContract.methods.deleteGoodUser(this.oneDelete.user).send({
