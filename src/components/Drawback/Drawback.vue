  ...
  <div class="page">
    <div class="all">
      ...
      <el-form :model="oneDonate" status-icon :rules="rules" ref="oneDonate" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item>
          <el-button type="primary" @click="drawback('oneDonate')">退回钱款</el-button>
        ...
        if (valid) {
          ...
          tokenContract.methods.ftype().call(null,(error,result)=>{
            if(error){
              ...
              if(result == 1){
                tokenContract.methods.t_money(this.oneDonate.addr).call(null, (error, result)=>{
                  this.money = result;
                })
              }
              else{
                tokenContract.methods.p_money(this.oneDonate.addr).call(null, (error, result)=>{
                  this.money = result;
                })
              }

              tokenContract.methods.drawback().send({
